...
public class Graph {
	...
	public int findVertex(String name) {
		...
		while (iter.hasNext()) {
			...
			if (iter.next().getFirst().getName().compareTo(name) == 0) {
				...
				// getName() returns the name of the City since that is the data
				// in the Node
				return i;
			} else
				i++;
	...
	public void addVertex(String n) {
		...
		int i = 0;
		// loop through the vertices array to see if the city you are looking
		// for is there
		ListIterator<LinkedList<City>> iter = vertices.listIterator();
		while (iter.hasNext()) {
			// iterate through the ArrayList checking the name of the city
		...
		nextVertexList = new LinkedList<City>(); // set up a new LinkedList for
													// this vertex
	...
	public boolean addEdge(String startCity, String endCity) {
		...
		if (positionSt == -1)
			...
			if (positionEnd == -1)
				...
				// the LinkedList contains the list of edges that can be reached
				// by one
	...
	public String displayAdjNodes(String startCity) {
		...
		if (position == -1)
			throw new RuntimeException("Vertex " + startCity
					+ " does not exist");
		else {
			edges = vertices.get(position);
			iter = edges.listIterator(); // get an iterator to the list from
											// that vertex
			while (iter.hasNext()) {
