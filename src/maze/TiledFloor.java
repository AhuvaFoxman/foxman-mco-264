...
public class TiledFloor {
	...
	public TiledFloor(int r, int c) {
		...
		// do this randomly , we don't know for sure whether our maze will be
		// traversable
		...
		String value = JOptionPane
				.showInputDialog("enter row,column of entrance");
		...
		if (row == 0 || row == rows - 1 || column == 0 || column == columns - 1) { // set
																					// entrance
			entrance = getTile(row, column);
			entrance.setColor(Color.WHITE); // mouse doesn't like to walk on red
											// tiles
			// now get the exit
			...
			if (row == 0 || row == rows - 1 || column == 0
				...
				exit.setColor(Color.WHITE); // mouse doesn't like to walk on red
											// tiles

			} else {
				JOptionPane.showMessageDialog(null,
						"incorrect exit specified, no such location");
			}

		} else {
			JOptionPane.showMessageDialog(null,
					"incorrect entrance specified, no such location");
		}
	...
	public void nextMoves(Tile fromSquare) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (tempRow >= 0 && tempRow < rows) {// check if column is in the
													// grid
				if (tempCol >= 0 && tempCol < columns) {
					if (!tiles[tempRow][tempCol].getVisited()) {
						...
						if (color == Color.white) {
							...
							// check if it is the exit, no need to search
							// further
							if (tiles[tempRow][tempCol].equals(exit))
								return;
							System.out.println("pushing a possible move "
									+ tiles[tempRow][tempCol]);
