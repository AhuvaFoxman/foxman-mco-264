...
public class SimulationValues {
	//
	/*
	 * SimulationValues maintains important simulation values. manages the
	 * simulation =============================================================
	 */
	...
	/***** Function Members *****/
	/***
	 * Constructor Postcondition: Output data members are initialized to 0 and
	...
	/***
	 * Input Input: Arrival rate, percentages for service times, and time limit
	 * for simulation Postcondition: Input data members are set.
	 * ---------------------------------------------------------
	 */
	...
	public void service() {
		...
		else if (!myIncomingCalls.isEmpty()) // calls are waiting -- get one
		{
			...
			totalWaitingTime += nextCall.getArrivalTime()
					- myTimer.timeRemaining();
		}
	}

	/***
	 * Output Output: total number of call and the average waiting time for
	 * calls ---------------------------------------------------------
	 */
	void display() {
		System.out
				.format("Number of calls processed: %d \nAverage waiting time per call: %4.2f \n ",
